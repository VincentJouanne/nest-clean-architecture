  ...
  execute(command: VerifyEmail): Promise<void> {
    const task = pipe(
      ...
      chain(([user, validatedDatas]) => {
        if (user == null) return left(new UserNotFoundException());
        else if (validatedDatas.verificationCode != user.contactInformation.verificationCode) return left(new IncorrectVerificationCodeException());
