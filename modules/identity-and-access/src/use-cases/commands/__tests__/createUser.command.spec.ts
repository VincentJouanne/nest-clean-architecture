...
describe('[Unit] Create user with an email', () => {
  ...
  beforeEach(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [InMemoryUUIDGeneratorService, InMemoryTagGeneratorService, InMemoryUserRepository, InMemoryLoggerService],
    }).compile();
    ...
    createUserHandler = new CreateUserHandler(uuidGeneratorService, tagGeneratorService, userRepository, logger);
  });
  ...
  it('OK - Should create a user if email is valid', async () => {
    ...
    const result = await createUserHandler.execute(new CreateUser(email));
  ...
  it('KO - Should not create a user if email is invalid', async () => {
    ...
    //When we create a user
    const resultPromise = createUserHandler.execute(new CreateUser(email));
  ...
  it('KO - Should not create a user if email already exists', async () => {
    ...
    await executeTask(
      userRepository.save(User.check({ id: uuidGeneratorService.generateUUID(), tag: tagGeneratorService.generateTag6(), email: email })),
    );
